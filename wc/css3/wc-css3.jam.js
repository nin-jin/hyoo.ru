    ...
    return function( nodeRoot ){
        ...
        return new function(){
            ...
            var nodeShape= $Lazy( function( ){
                var node= $Node.parse( '<vml:shape style=" position: absolute; display: block; width: 100%; height: 100%; left: 0; top: 0; z-index: -1" />' ).$
                var type= nodeRoot.currentStyle[ '-wc-css3_shapetype' ] || 'wc-css3_roundrect'
                node.setAttribute( 'type', node.type= ( '#' + type ) )
                shaped= true
                nodeRoot.insertBefore( node, nodeRoot.firstChild )
                return $Value( node )
            })
